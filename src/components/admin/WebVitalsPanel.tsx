  ...
  return (
    <div className="space-y-6">
                ...
                {topPaths.map(({ path, total, metrics }) => (
                  <tr key={path} className="border-t border-border">
                    ...
                    {METRICS.map((m) => {
                      ...
                      const p75 = percentile(sorted, 0.75);
                      if (p75 == null) return <td key={m} className="text-right px-3 py-2 text-muted-foreground">—</td>;
      ...
      <p className="text-xs text-muted-foreground">
        Metrics are only recorded on production builds (your published site). Each visit emits one sample per metric. p75 = the value 75% of your visitors experience or better — Google's official ranking benchmark.
      </p>
    </div>
  );
};
