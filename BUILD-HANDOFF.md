...
- The homepage now uses current Big Boys imagery with descriptive alternative text and deferred decoding/loading for noncritical images.
...
4. Replace externally referenced Big Boys images with approved, optimized local copies if the production host requires asset self-hosting.
5. Delete or redirect the legacy SeaBee-only files that are not in the sitemap before the domain switch; they are intentionally not part of the new Big Boys route architecture.
